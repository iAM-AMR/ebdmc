...
#'
#' @title
#'    \emph{Salmonella} Heidelberg isolates typed using a cgMLST scheme at different levels of stringency
#'
#' @description
...
#'    broiler chickens were typed using a cgMLST scheme at different levels of
#'    stringency.
#'
...
#'
#' @format
#'    A data frame (tibble) of 257 rows and 38 columns:
#'    \describe{
#'      \item{id}{an anonymized identifier.}
#'      \item{group}{the production stage from which the isolate was sampled.}
#'      \item{cgmlst_###}{the cluster/type membership using a stringency of ###,
#'                        where ### is the threshold number of loci difference
#'                        used to establish a new type. For example, where 000,
#'                        isolates which differ at any loci are considered a new
#'                        type. Where 001, isolates which differ at more than one
#'                        loci are considered a new type.}
#'    }
